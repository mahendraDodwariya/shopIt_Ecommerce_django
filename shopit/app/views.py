...
def search(request):
    ...
    return render(request,'app/searcheditem.html',{'items':items})
...
def mobile(request,data = None):
    if(data == None):
        ...
        mobiles = Product.objects.filter(category = 'M').filter(discounted_price__gt=9999)

    return render(request, 'app/mobile.html',{'mobiles':mobiles  , 'active':'btn-primary'})

 
def laptop(request,data = None):
    if(data == None):
        laptops = Product.objects.filter(category='L')
    elif(data == 'Lenevo' or data == 'Apple'):
        laptops = Product.objects.filter(category = 'L').filter(brand = data)
    elif(data == 'Below'):
        laptops = Product.objects.filter(category = 'L').filter(discounted_price__lt=50000)
    elif(data == 'Above' ):
        laptops = Product.objects.filter(category = 'L').filter(discounted_price__gt=50001)
...
def topwear(request,data = None):
    if(data == None):
        ...
        topwears = Product.objects.filter(category = 'TW').filter(discounted_price__gt=501)
...
def bottomwear(request,data = None):
    if(data == None):
        ...
        bottomwears = Product.objects.filter(category = 'BW').filter(discounted_price__gt=501)
